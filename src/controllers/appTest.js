...
var pokeJSONFile = fs.readFileSync("".concat(root, "/src/models/pokemonNames.json"), "utf8");
// 파싱 된 포켓몬 데이터
var parsedPoke = JSON.parse(pokeJSONFile);
// client input 포켓몬 데이터 처리 함수
function findPokeMoudle(urlValue) {
    var result = parsedPoke.filter(function (x) {
        if (x === urlValue) {
            return true;
        }
    });
    ...
    try {
        if (req.method === "GET") {
            ...
            if (req.url && req.url.includes("/name")) {
                // console.log(req.url)
                var parseData = qs.parse(req.url, { ignoreQueryPrefix: true });
                // console.log(parseData);
                var valueData = Object.values(parseData);
                // console.log(valueData[0]);
                var printData = findPokeMoudle(valueData[0]);
                console.log("모듈 결과 값" + printData);
                // res.writeHead(200, {'Content-Type' : 'text/html'})
                // res.write("hello");
                res.end("".concat(printData));
