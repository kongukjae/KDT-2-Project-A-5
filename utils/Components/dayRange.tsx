...
const DayRange = (): JSX.Element => {
  ...
  console.log("aaplData['symbol']", aaplData[0].symbol)
  ...
  fetch(`/yesterDayDataRequest`, {
    ...
    // split('-')
    body: JSON.stringify({ stockName: aaplData[0].symbol , day: formattedDate }),
  })
    .then((response) => {
      if (response.ok) {
    ...
    .then((data) => {
      console.log("데이터 잘 들어오나 확인",data)
