...
import StockContentBox from "./stockContentsBoxLayout";

// 함수의 컴포넌트 타입을 명시하기 위해서이다.
// 프로퍼티의 키 값이 무엇이 들어 오든 다 받을수 있다.
interface stockContentsBoxProps {
  [key: string]: string;
}

const StockContentsBox: React.FC<stockContentsBoxProps> = (text) => {
  ...
  // 태그 추가 하는 곳
  useEffect(() => {
    addTagToLayout(<StockContentBox />);
    addTagToLayout(<StockContentBox />);
    addTagToLayout(<StockContentBox />);
    addTagToLayout(<StockContentBox />);
    addTagToLayout(<StockContentBox />);
...
export default StockContentsBox;
